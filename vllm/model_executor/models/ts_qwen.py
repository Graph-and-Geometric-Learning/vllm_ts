...
Adapted for vLLM v0.11.0 by Aosong Feng
"""
...
# Debug mode for saving intermediate tensors (set via environment variable)
DEBUG_MODE = os.environ.get("TS_QWEN_DEBUG", "0") == "1"
DEBUG_OUTPUT_DIR = "/fsx/ubuntu/users/aosong/fin/fin_ts_llm/debug_outputs/vllm"

# Path to synced weights from HF model (for debugging)
DEBUG_SYNC_WEIGHTS_PATH = "/fsx/ubuntu/users/aosong/fin/fin_ts_llm/debug_outputs/hf_ts_weights.pt"

# Debug mode for setting breakpoints (set via environment variable DEBUG_VLLM=1)
DEBUG_VLLM = os.environ.get("DEBUG_VLLM", "0") == "1"


def debug_breakpoint(location: str):
    """
    Set a breakpoint if DEBUG_VLLM=1 environment variable is set.

    Usage: DEBUG_VLLM=1 python your_script.py

    Args:
        location: Description of where this breakpoint is (for logging)
    """
    if DEBUG_VLLM:
        print(f"\n{'='*60}")
        print(f"[DEBUG_VLLM] Breakpoint at: {location}")
        print(f"{'='*60}")
        # import pdb; pdb.set_trace()


def debug_save(name: str, tensor: "torch.Tensor"):
    """Save tensor to numpy file if debug mode is enabled."""
    if DEBUG_MODE and tensor is not None:
        os.makedirs(DEBUG_OUTPUT_DIR, exist_ok=True)
        filepath = os.path.join(DEBUG_OUTPUT_DIR, f"{name}.npy")
        np.save(filepath, tensor.detach().float().cpu().numpy())
        print(f"  [DEBUG-VLLM] Saved {name}: {tensor.shape}")
...
class TS2QwenDummyInputsBuilder(BaseDummyInputsBuilder[TS2QwenProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        """Build dummy multimodal data for memory profiling (v0.11.0 API)."""
...
class TS2QwenMultiModalProcessor(BaseMultiModalProcessor[TS2QwenProcessingInfo]):
    ...
    def _call_hf_processor(
        ...
        # BREAKPOINT 1: After tokenization, before processing timeseries data
        # This is called by vLLM's BaseMultiModalProcessor.apply() during input preprocessing
        # See: vllm/multimodal/processing/processor.py:1840 (_cached_apply_hf_processor)
        # You can inspect: prompt, prompt_ids, mm_data (contains raw timeseries tensor)
        debug_breakpoint("_call_hf_processor: After tokenization (STEP 1 - Input Preprocessing)")
    ...
    def _get_prompt_updates(
        ...
        # BREAKPOINT 2: After defining placeholder replacement rules
        # This is called by vLLM's BaseMultiModalProcessor._maybe_apply_prompt_updates()
        # See: vllm/multimodal/processing/processor.py:1850
        # The single <|timeseries_pad|> token will be replaced by num_soft_tokens copies
        # You can inspect: ts_token_id, num_soft_tokens, target/replacement logic
        debug_breakpoint("_get_prompt_updates: Placeholder replacement rules (STEP 2 - Token Replacement)")
...
class TS2QwenForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _encode_timeseries(self, timeseries: torch.Tensor) -> torch.Tensor:
        ...
        # KEY DEBUG POINT 1: Soft tokens [5, 1024] (only save during prefill)
        if soft_tokens.shape[0] == 1:  # Single batch during inference
            debug_save("1_soft_tokens", soft_tokens.squeeze(0))  # [5, 1024]

        # BREAKPOINT 4: After timeseries encoding, soft tokens ready
        # This is where raw timeseries [B, L, C] becomes soft tokens [B, num_channels, hidden_size]
        # The soft_tokens will be scattered into text embeddings by _merge_multimodal_embeddings
        # See: vllm/model_executor/models/utils.py:438 (_merge_multimodal_embeddings)
        # You can inspect: timeseries.shape, soft_tokens.shape, ts_embeddings intermediate values
        debug_breakpoint("_encode_timeseries: Soft tokens generated (STEP 4 - TS Encoding Complete)")
    ...
    def _process_timeseries_input(
        ...
        debug_save("6_text_embeddings", inputs_embeds)
        ...
        concat_embeds = torch.cat([soft_tokens, inputs_embeds], dim=1)
        debug_save("7_concat_embeddings", concat_embeds)
    ...
    def forward(
        ...
        **kwargs,
        ...
        NOTE: In vLLM's multimodal pipeline, this forward() is typically called with
        inputs_embeds already containing merged text+multimodal embeddings (via embed_input_ids).
        The 'timeseries' kwarg path below is for alternative usage patterns.
        """
        timeseries = kwargs.pop("timeseries", None)
        # import pdb; pdb.set_trace()
        ...
        # Only save during prefill (seq_len > 1) to avoid overwriting with decode steps
        is_prefill = inputs_embeds is not None and inputs_embeds.shape[0] > 1

        if is_prefill:
            # KEY DEBUG POINT 2: Inputs to LLM [seq_len, hidden] and position IDs
            debug_save("2_inputs_embeds", inputs_embeds)
            debug_save("2_position_ids", positions)

        # Run through language model
        # Note: Debug saves for 3_after_first_layer and 4_last_hidden_states
        # are now inside qwen2.py's Qwen2Model.forward() - no workaround needed
